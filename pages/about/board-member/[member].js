...
const Member = () => {
  ...
  const { boardMembers } = useContext(MembersContext);
  const memberName = router.query.member;
  const memberObj = boardMembers.find((item) => item.id === memberName);
  ...
  return (
    <div className="bg-[#F5F5F5] overflow-x-hidden">
      ...
      <section className="max-w-[1400px] mx-auto px-5 md:px-10 lg:px-20 mt-10 sm:pb-16 ">
