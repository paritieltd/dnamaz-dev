...
const SingleBlog = ({ oneBlog }) => {
  // Prevent execution if no blog data
  if (!oneBlog) {
    return <div className="text-center text-xl mt-10">Content is currently unavailable.</div>;
  }
    ...
    <>
      <div className="bg-[#C8C8C8]/50 text-[#2C393F]">
        ...
        <div className="max-w-[1200px] sm:mx-2 md:mx-4 lg:mx-28 p-4 ">
          ...
          {/* Rendering blocked */}
          {/* <img className="mt-6 w-[100%] mx-auto" src={imageUrl} alt={alt_text} /> */}
          {/* <div className="sm:text-xl mt-8" dangerouslySetInnerHTML={{ __html: content?.rendered }}></div> */}
        </div>
        <Subscribe />
        <Footer />
      </div>
